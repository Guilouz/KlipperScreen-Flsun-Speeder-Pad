...
printer_1_data_config = os.path.join(home, "printer_1_data", "config") # Changes
...
class KlipperScreenConfig:
    ...
    def _create_configurable_options(self, screen):
        ...
        for num in SCREEN_BLANKING_OPTIONS:
            ...
            minute = num // 60
            if hour > 0:
                name = f'{hour} ' + ngettext("hour", "hours", hour) # Changes
            else:
                name = f'{minute} ' + ngettext("minute", "minutes", minute) # Changes
    ...
    def get_config_file_location(self, file):
        ...
        directories = [printer_data_config, printer_1_data_config, xdg_config, klipperscreendir] # Changes
